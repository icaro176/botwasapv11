...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `I ala o @${num.split('@')[0]} saiu e nem fez falta kakaka 🤣`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
          ...
          const checkLimit = (sender) => {
          	let found = false
                    for (let lmt of _limit) {
                        if (lmt.id === sender) {
                            ...
                            if (limitCounts <= 0) return client.sendMessage(from,`Seu limite acabou\n\n_Nota : limite pode ser obtido por meio de ${prefix}buylimit <total> e subindo de nível_`, text,{ quoted: mek})
            ...
            case 'bitly':
               ...
               if (data.error) return reply(data.error)
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("|")[1];
					const pref = `Usage: \n${prefix}quotemaker texto|marca d'água\n\nEx :\n${prefix}quotemaker baiano|bot`
                ...
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                    ...
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return client.sendMessage(from, 'O texto é longo, no máximo 10 letras', text, {quoted: mek})
               ...
               case 'help': 
         	   ...
         	   case 'creator':
                  ...
                  client.sendMessage(from, 'Este é o número do meu proprietário >_<',MessageType.text, { quoted: mek} )
					break    
				case 'leaderboard':
				case 'lb':
				_level.sort((a, b) => (a.xp < b.xp) ? 1 : -1)
				uang.sort((a, b) => (a.uang < b.uang) ? 1 : -1)
                let leaderboardlvl = '-----[ *TOP 10 MELHORES EM NIVEL 😎* ]----\n\n'
                let leaderboarduang = '-----[ *TOP 10 MELHORES EM DINHEIRO 😎* ]----\n\n'
                let nom = 0
                try {
                    for (let i = 0; i < 10; i++) {
                        nom++
                ...
                addKoinUser('994406786500@s.whatsapp.net', fee)
				...
				case 'slap':
                    ...
                    if (isLimit(sender)) return reply(ind.limitend(pusname))
					const slap =['cachorro','seu porco','seu cachorro','estúpido','cara para bolo de macaco','tá solteiro sozinho de novo idiota','então eu só quero ter uma namorada cara, o macaco sai da gaiola','Simplesmente bonito, apenas chamado de tóxico','Sua calcinha','Seu desgraçado','por tudo você chora','toda a sua buc*t*','vai cachorro','bonitos só escolhe mulher casada','Você é lindo, cachorro crocodilo macho','filho da puta','toma','o cachorro vem uma luta','fantasmas com medo de ser gentil','apenas péssimo, não é difícil de quebrar','enfia seu p@# em mim 😏','menino, você é todo idiota','Mais 3 dias']
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					const bisa =['Pode','Não pode','Tente repetir']
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					kapankah = body.slice(1)
					const kapan =['Amanhã','Lusa','Agora mesmo','restam 4 dias','Mais 5 dias','Mais 6 dias','1 semana novamente','2 semanas de novo','3 semanas de novo','Mais 1 mês','Mais 2 meses','Mais 3 meses','Mais 4 meses','Mais 5 meses','Mais 6 meses','Mais 1 ano','Mais 2 anos','Mais 3 anos','Mais 4 anos','Mais 5 anos','Mais 6 anos','1 século novamente','Mais 3 dias']
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
					apakah = body.slice(1)
					const apa =['sim','Não','claro','Tente repetir']
                ...
                const trut =['Você já gostou de alguém?','Se você tivesse uma máquina do tempo, para que época você iria?','Qual é o seu maior medo?','Você já gostou de alguém e sentiu que essa pessoa também gostava de você?','Qual é o nome da ex-namorada do seu amigo que você gostava secretamente?','Você já roubou o dinheiro de sua mãe ou de seu pai? O motivo?','o que te faz feliz quando você está triste?','amor nunca correspondido? se alguma vez com quem? como se sente, brow?','já teve um caso com alguém?','Você já foi hipnotizado?','quem é a pessoa mais influente em sua vida','que coisa orgulhosa você teve este ano','quem é a pessoa que pode fazer você se sentir desconfortável','quem é a pessoa que te deixou desconfortável','já orou diariamente?','Se tivesse que escolher entre inteligência e beleza, o que você escolheria?','Com quem você gosta de jogar?','você já recusou alguém? a razão porque?','Mencione o acidente que te machucou e você ainda se lembra','Que conquistas você obteve este ano?','qual era o seu pior hábito na escola?','Qual foi o pior presente que você já ganhou?','Se fizessem um filme da sua vida, quem interpretaria você?','Quantas vezes você saiu de casa escondido?','Se você pudesse morar em qualquer lugar do mundo, onde moraria?','Qual foi o melhor dia da sua vida?','Você pratica algum esporte?','Você já sentiu atração por algum professor? Quem? Por quê?','Você tem um diário?','Você se cadastraria em um site de namoro?','Qual foi a peça mais cruel que você já pregou em alguém?','Qual era o seu desenho preferido quando você era criança?','Qual foi o seu pior beijo?','Você preferiria ser inteligente ou feliz, e por quê?','Você já pegou alguma DST?','Quando você aprendeu a andar de bicicleta?','Se dinheiro não existisse, o que você faria da vida?','Você dorme pelado?','Você já quebrou alguma coisa e botou a culpa em outra pessoa?','Você já deixou alguém entrar em casa escondido?','Qual é a coisa mais esquisita sobre você? Você se orgulha disso?','Qual é o seu personagem da Disney preferido?','O que você faria se fosse invisível por um dia?','Quem você levaria desse grupo para uma ilha deserta?','O que você faria com 1 milhão de reais agora?','Qual é a sua maior vergonha na vida?','Se você pudesse mudar algo em você, o que seria?','Conte um segredo ou uma curiosidade sua','É verdade que você já ficou com alguém desse grupo?','Biscoito ou bolacha? Defenda sua teoria.','O que os olhos não vêem o coração não sente?','Se o mundo fosse acabar e você pudesse salvar apenas uma pessoa para sobreviver, quem seria?','É possível morrer de amor?','Como foi o seu primeiro beijo?','É verdade que você ficaria com alguém que está presente aqui no grupo?','Para onde você viajaria agora?','Mãe ou pai?','Em uma máquina do tempo, para que época da história ou momento da sua vida você voltaria?','Você participaria do BBB?']
					...
					truteh = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSG7IJRxTdpUBuy0zY7KztGdw2mk6IPB7xEIA&usqp=CAU`)
				...
				const dare =['Envie uma mensagem para o seu/sua ex e diga "Eu ainda gosto de você"','Trocar o status de relacionamento no Facebook.','diga "eu te amo" para algum membro do grupo','Não sair do celular até o final da brincadeira.','sua chamada recente do Whatsapp','Postar uma foto feia nas redes sociais, depois disso poste a print aqui no grupo e deixe a foto por 24h.','Escolher uma música para cantar em áudio até o final sem errar.','Escolha alguém para publicar uma mensagem qualquer em seu Facebook.','Ligar para alguém que por algum motivo a pessoa não conversa há muito tempo.','Mastigar um alho em vídeo.','Crie uma música de rap usando nome de todos que fazem parte da brincadeira.','Simular um pedido de casamento.','Tomar 1 litro de água no menor tempo possível.','Revelar a última mensagem que você mandou no WhatsApp.','Mude a sua foto de perfil no Facebook com a sua pior foto.','Envie para algum contatinho alguma das melhores cantadas do WhatsApp','Fingir que está na balada e mostrar como você faria para chegar em alguém','Mostrar as suas últimas mensagens do WhatsApp aqui no grupo.','Mostre a última foto que você tirou de você mesmo no celular.','Mostre para a galera qual foi o último vídeo que você assistiu no YouTube','Dar uma aula de axé anos 90.','Curtir fotos antigas de uma pessoa aleatória que você não segue no Instagram.','Mande uma mensagem para uma pessoa que você não conversa há mais de um ano.','Passar o resto do jogo se comportando como um animal que o grupo escolheu.']
					...
					tod = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSG7IJRxTdpUBuy0zY7KztGdw2mk6IPB7xEIA&usqp=CAU`)
                ...
                if (!isRegistered) return reply(ind.noregis())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                const eununca =['Eu nunca… passei um trote para a polícia. (Péssima ideia!)','Eu nunca… encontrei uma celebridade.','Eu nunca… conversei por chamada de voz por quatro horas seguidas.','Eu nunca… acordei bêbado.','Eu nunca… roubei algo que custasse mais de R$10.','Eu nunca… mandei uma selfie sexy para alguém.','Eu nunca… dei em cima de alguém enquanto estava em um encontro com outra pessoa.','Eu nunca… dei PT em um carro.','Eu nunca… gostei de uma música do Justin Bieber.','Eu nunca… tive um amigo que se virou contra mim.','Eu nunca… tive rodinhas na bicicleta.','Eu nunca… estraguei as férias alheias.','Eu nunca… dormi na aula e acordei no meio de uma poça de baba.','Eu nunca… bebi antes de fazer 18 anos.','Eu nunca… apostei em alguma coisa.','Eu nunca… quis matar alguém.','Eu nunca… tomei um shot do corpo de alguém.','Eu nunca… andei de moto.','Eu nunca… matei aula.','Eu nunca… dormi no trabalho.','Eu nunca… me atrasei para o trabalho ou para a aula.','Eu nunca… colei em uma prova.','Eu nunca… fiz bullying com alguém.','Eu nunca… flertei com um professor.','Eu nunca… fingi que estava doente para faltar à escola ou ao trabalho.','Eu nunca… sofri bullying.','Eu nunca… xinguei um professor.','Eu nunca… beijei alguém na escola.','Eu nunca… caí de tanto beber.','Eu nunca… tomei um shot.','Eu nunca… gastei mais de $500 numa noitada.','Eu nunca… bebi uísque como se fosse água.','Eu nunca… desmaiei de tanto beber.']
					const eununca = trut[Math.floor(Math.random() * trut.length)]
					truteh = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS8bvflQkNX1WtThL-mxJBi8qJjChJNPYITTQ&usqp=CAU`)
					client.sendMessage(from, truteh, image, { caption: '*Eu Nunca... Eu Já...*\n\n'+ ttrth, quoted: mek })
					await limitAdd(sender)
					break
                                        case 'eununca2':
                if (!isRegistered) return reply(ind.noregis())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                const eununca2 =['Eu nunca… transei com alguém porque fui desafiado.','Eu nunca… fui a um sex shop.','Eu nunca… vi meus pais pelados.','Eu nunca… tive fantasias com outra pessoa.','Eu nunca… nadei pelado.','Eu nunca… fiz swing.','Eu nunca… chorei durante o sexo.','Eu nunca… levei um susto porque achei que estava grávida.','Eu nunca… observei alguém se masturbar.','Eu nunca… flagrei meus pais durante o sexo.','Eu nunca… senti atração pela mãe de um amigo.','Eu nunca… tirei nudes.','Eu nunca… joguei strip poker.','Eu nunca… tomei um shot do corpo de alguém.','Eu nunca… transei com um estranho.','Eu nunca… fiz ou recebi um footjob.','Eu nunca… usei lubrificante.','Eu nunca… me masturbei no banho.','Eu nunca… me aliviei sozinho depois do sexo porque não estava satisfeito.','Eu nunca… cheirei a calcinha usada de alguém.','Eu nunca… tomei banho com um amigo.','Eu nunca… usei um brinquedo sexual de outra pessoa.','Eu nunca… flertei com alguém para fazer ciúmes em outra pessoa.','Eu nunca… tentei usar música para criar um clima.','Eu nunca… fiz vídeos ousados.','Eu nunca… recebi uma reclamação de barulho por causa do sexo.','Eu nunca… usei viagra por curiosidade.','Eu nunca… namorei mais de uma pessoa ao mesmo tempo.','Eu nunca… tive fantasias com o parceiro de um amigo.','Eu nunca… tentei roubar o parceiro de alguém.','Eu nunca… experimentei uma camisinha com sabor.','Eu nunca… me apaixonei por alguém que eu não conhecia.','Eu nunca… rejeitei alguém por ter muitos pelos no corpo.','Eu nunca… fingi que gostava de algo só porque meu crush gostava.','Eu nunca… namorei a mesma pessoa duas vezes.']
					const eununca2 = trut[Math.floor(Math.random() * trut.length)]
					truteh = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS5g5DP0Tx4VZ80HYb2vKzYrgl_qczsXzOnrQ&usqp=CAU`)
					client.sendMessage(from, truteh, image, { caption: '*Eu Nunca... Eu Já... 2*\n\n'+ ttrth, quoted: mek })
					await limitAdd(sender)
					break
				case 'ocr': 
				if (!isRegistered) return reply(ind.noregis())
				...
				if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('mande uma foto que apareça letras, com a legenda ${prefix}ocr')
					}
					await limitAdd(sender)
				break
				case 'stiker': 
				...
				case 's':
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(`Envie foto com a legenda ${prefix}sticker ou marque uma imagem`)
					}
				break
				case 'tts':
				if (!isRegistered) return reply(ind.noregis())
               ...
               client.sendMessage(from, resul, text, { quoted: mek})
				...
				case 'mining':
                      ...
                      if (!isEventon) return reply(`Desculpe ${pushname} evento de mineração é ativada pelo proprietário`)
                      if (isOwner) {
                      const one = 999999999
                      addLevelingXp(sender, one)
                      addLevelingLevel(sender, 99)
                      reply(`pega ${one}XP pra vc ${pushname} ❤`)
                      }else{
                ...
                client.sendMessage(from, 'Descrição do Grupo Alterada', text, {quoted: mek})
					break
           case 'demote':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					...
					if (mentioned.length > 1) {
						...
						mentions(`o @${mentioned[0].split('@')[0]} perdeu adm🤣`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`o @${mentioned[0].split('@')[0]} *virou adm 😳*`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Kakakaka foi removido :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`kakakak o @${mentioned[0].split('@')[0]} foi removido`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
					if (!isGroup) return reply(ind.groupo())
					teks = `Total de adm do grupo *${groupMetadata.subject}*\n𝗧𝗼𝘁𝗮𝗹 : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'welcome':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (args.length < 1) return reply('coloca ${prefix}welcome 1 fdp')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('*ATIVADO* !!!')
						welkom.push(from)
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						reply('❬ SUCESSO ❭ ativou modo boas vindas nesse grupo')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						reply('❬ SUCESSO ❭ desativou modo boas vindas nesse grupo')
					} else {
						reply(ind.satukos())
					}
					break 
					...
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (args.length < 1) return reply('coloca ${prefix}simih 1 fdp')
					if (Number(args[0]) === 1) {
						if (isSimi) return reply('ativado !!!')
						samih.push(from)
						fs.writeFileSync('./database/bot/simi.json', JSON.stringify(samih))
						reply('❬ SUCESSO ❭ ativou modo simih')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./database/bot/simi.json', JSON.stringify(samih))
						reply('❬ SUCESSO ❭ desativou modo simih')
					} else {
						reply(ind.satukos())
					}
					break
				case 'nsfw':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (args.length < 1) return reply('coloca ${prefix}nsfw 1 fdp')
					if (Number(args[0]) === 1) {
						if (isNsfw) return reply(' *ativado*  !!')
						nsfw.push(from)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						reply('❬ SUCESSO ❭ ativou modo nsfw')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						reply('❬ SUCESSO ❭ desativou modo nsfw')
					} else {
						reply(ind.satukos())
					}
				break
                ...
                if (!isGroup) return reply(ind.groupo())
                if (!isGroupAdmins) return reply(ind.admin())
                if (args.length < 1) return reply('coloca ${prefix}leveling enable fdp')
                if (args[0] === 'enable') {
				...
				case 'clone':
			  	...
			  	case 'event':
					if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
					if (args.length < 1) return reply('coloca ${prefix}event 1 fdp')
					if (Number(args[0]) === 1) {
						if (isEventon) return reply('*ATIVADO* !!!')
						event.push(from)
						fs.writeFileSync('./database/bot/event.json', JSON.stringify(event))
						reply('*❬ SUCESSO ❭ ativou modo evento*')
					} else if (Number(args[0]) === 0) {
						event.splice(from, 1)
						fs.writeFileSync('./database/bot/event.json', JSON.stringify(event))
						reply('*❬ SUCESSO ❭ desativou modo evento*')
					} else {
						reply(ind.satukos())
					}
					break
				...
				case 'setpp': 
                       ...
                       media = await client.downloadAndSaveMediaMessage(mek)
                        ...
                        reply('foto de perfil do grupo alterado')
					break				
				case 'leave': 
				if (!isGroup) return reply(ind.groupo())
				if (!isOwner) return reply(ind.ownerb())
				await reply(from, 'tchau').then(() => client.groupLeave(groupId))
					break
				case 'bc': 
					if (!isOwner) return reply(ind.ownerb()) 
				...
				case 'setmemlimit':
                    ...
                    memberlimit = args[0]
                    reply(`limite do membro alterado para ${memberlimit}`)
				break 
				case 'bcgc':
				     if (!isOwner) return reply(ind.ownerb())
					if (args.length < 1) return reply('.......')
